...
// NAV + PROGRESS
const ids   = ['cover','ch-1','ch-2','ch-3','ch-4','ch-5','ch-6','ch-ep'];
...
const obs  = new IntersectionObserver(e=>{
  e.forEach(r=>{if(r.isIntersecting){r.target.classList.add('visible');obs.unobserve(r.target);}});
},{threshold:.1});
...
function initB(){
  ...
  pts=Array.from({length:60},()=>({x:Math.random()*bW,y:Math.random()*bH,r:Math.random()*.7+.2,a:Math.random()*.25+.05,sp:.001+Math.random()*.003,ph:Math.random()*Math.PI*2}));
}
function drawB(t){
  ...
  pts.forEach(p=>{
    ...
    bx.fillStyle='rgba(180,220,200,1)';bx.fill();
  });
